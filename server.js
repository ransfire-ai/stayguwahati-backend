...
// Resolve a stable User/ObjectId for public host-profile links. Older
// properties may only have the host email, so this also backfills the
// identity in the API response without requiring a database migration.
const enrichHostIdentity = async (listing) => {
    const item = { ...listing };
    const existingHost = item.host && typeof item.host === 'object' ? { ...item.host } : {};

    let hostId = existingHost.hostId || existingHost._id || existingHost.id || item.hostId || item.ownerId || null;
    const hostEmail = String(
        item.ownerEmail ||
        existingHost.email ||
        item.hostEmail ||
    ...
    if (!hostId && hostEmail) {
        try {
            const user = await User.findOne({ email: hostEmail }).select('_id').lean();
            if (user?._id) hostId = user._id;
        } catch (lookupError) {
            console.warn('[HOST PROFILE] Could not resolve host identity:', lookupError.message);
        }
    }

    if (hostId) {
        item.hostId = String(hostId);
        item.ownerId = String(item.ownerId || hostId);
        item.ownerEmail = item.ownerEmail || hostEmail;
        item.host = { ...existingHost, hostId: String(hostId) };
    } else if (item.host) {
        item.host = existingHost;
    }
...
const getHomestaysHandler = async (req, res) => {
    try {
        ...
        const identityListings = await Promise.all(listings.map(enrichHostIdentity));

        const optimizedListings = identityListings.map(listing => ({
            ...listing,
...
const getSingleHomestayHandler = async (req, res) => {
    try {
        ...
        // Resolve a stable public host identity for the Host Profile link.
        const enrichedHomestay = await enrichHostIdentity(homestay);
        Object.assign(homestay, enrichedHomestay);
...
app.post('/api/homestays', authenticateToken, async (req, res) => {
    try {
        ...
        const actorUserId = mongoose.Types.ObjectId.isValid(String(req.user?.userId || ''))
